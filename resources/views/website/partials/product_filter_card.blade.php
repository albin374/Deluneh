        ...
        <div class="relative w-full overflow-hidden filter-container">
            <div class="h-full w-full relative">
                @if($product->images && count($product->images) > 0)
                    <img alt="{{ $product->name }}" class="filter-img w-full rounded-lg" loading="lazy" decoding="async" src="{{ asset('storage/' . $product->images[0]) }}" style="{{ $section->zoom_out_image ? 'object-fit: contain;' : 'aspect-ratio: 3/4; object-fit: cover;' }}">
                @else
                    <img alt="{{ $product->name }}" class="filter-img w-full rounded-lg" loading="lazy" decoding="async" src="/images/f3.webp" style="{{ $section->zoom_out_image ? 'object-fit: contain;' : 'aspect-ratio: 3/4; object-fit: cover;' }}">
                @endif
            </div>
        </div>
    </a>
